...
int main(int argc, char const* argv[]) {
    ...
    skcp_client_t* serv = skcp_client_init(loop, tcp_ip, tcp_port, udp_ip, udp_port, &conf);
    assert(serv);
    /* skcp_client_set_cb(serv, skcp_rcv_cb, skcp_client_auth_cb); */

    _LOG("client start, udp listening on %s %u, tcp listenging on %s %u\n", udp_ip, udp_port, tcp_ip, tcp_port);

    ev_run(loop, 0);

    skcp_client_free(serv);
