...
static void on_server_udp_rcv(struct ev_loop *loop, struct ev_io *watcher, int revents) {
    ...
    char *out = NULL;
    int out_len = 0;
    int ret = skcp_input(g_skcp, g_rcv_buf, rlen, &cid, &out, &out_len);
    assert(ret == _OK);
    _LOG("rcv:%s len:%d", out, out_len);
    ret = skcp_send(g_skcp, cid, out, out_len);
    assert(ret > 0);
}
static void on_client_udp_rcv(struct ev_loop *loop, struct ev_io *watcher, int revents) { /* TODO: */ }

static void on_udp_rcv(struct ev_loop *loop, struct ev_io *watcher, int revents) {
    _CHECK_EV;
    if (g_skcp->conf.mode == SKCP_MODE_SERV) return on_server_udp_rcv(loop, watcher, revents);
    return on_client_udp_rcv(loop, watcher, revents);
}
...
int main(int argc, char const *argv[]) {
    ...
    if (g_skcp->conf.mode == SKCP_MODE_SERV) {
        skcp_conn_t *newconn = skcp_init_conn(g_skcp, 1, target_sockaddr); /* TODO: dynamically init conn*/
        assert(newconn);
        ...
        ev_timer_set(&update_watcher, kcp_interval, kcp_interval);
