...
//     int buf_len;
//     char buf[];
// } skcp_udp_pipe_data_t;

// static int kcp_output(const char *buf, int len, struct IKCPCB *kcp, void *user) {
//     skcp_conn_t *conn = (skcp_conn_t *)user;

//     // skcp_udp_pipe_data_t *pd;
//     // pd = _ALLOC(sizeof(skcp_udp_pipe_data_t) + len);
//     // pd->skcp = conn->skcp;
//     // pd->dest_addr = conn->dest_addr;
//     // pd->buf_len = len;
//     // memcpy(pd->buf, buf, len);
//     skcp_udp_pipe_data_t *pd;
//     const int pd_size = sizeof(skcp_udp_pipe_data_t) + len;
//     ssize_t bytes = write(udp_sender_pipe_fd[1], &pd_size, sizeof(int));
//     if (bytes < 0) {
//         // _LOG("write udp sender pipe error %ld", bytes);
//         perror("write udp sender pipe error");
//         return bytes;
//     }
//     pd = _ALLOC(pd_size);
//     // pd->size = pd_size;
...
//     pd->buf_len = len;
//     memcpy(pd->buf, buf, len);
//     // char *pd_buf = _ALLOC(pd_size);
//     bytes = write(udp_sender_pipe_fd[1], pd, pd_size);
//     _FREEIF(pd);
//     if (bytes < 0) {
//         // _LOG("write udp sender pipe error %ld", bytes);
//         perror("write udp sender pipe error");
...
//     int32_t bytes = recvfrom(skcp->fd, raw_buf, skcp->conf->r_buf_size, 0, (struct sockaddr *)&cliaddr,
//     &cliaddr_len); if (-1 == bytes) {
...
// static void *udp_sender_thread_func() {
//     while (1) {
//         int pd_size = 0;

//         int r = read(udp_sender_pipe_fd[0], &pd_size, sizeof(int));
//         if (r < 0) {
//             perror("udp_sender_thread_func read pd_size error");
//             continue;
//         }
//         // _LOG("udp_sender_thread_func read pd_size: %d", pd_size);
//         skcp_udp_pipe_data_t *pd = _ALLOC(pd_size);
//         r = read(udp_sender_pipe_fd[0], pd, pd_size);
//         if (r < 0) {
//             perror("udp_sender_thread_func read pd_size error");
//             continue;
//         }

//         int rt = udp_send(pd->skcp, pd->buf, pd->buf_len, pd->dest_addr);
//         if (rt < 0) {
//             _LOG("udp_sender_thread_func udp_send error");
//         }
//     }
...
//     // int fd[2];
//     if (pipe(udp_sender_pipe_fd) < 0) {
//         _LOG("udp sender pipe error");
//         skcp_free(skcp);
//         return NULL;
//     }
//     pthread_t udp_sender_tid = 0;
//     int terr = pthread_create(&udp_sender_tid, NULL, udp_sender_thread_func, NULL);
//     if (terr) {
//         _LOG("udp_sender_thread start error");
//         skcp_free(skcp);
//         return NULL;
//     }
//     _LOG("udp_sender_thread start ok");
